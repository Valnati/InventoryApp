...
public class BookProvider extends ContentProvider{
    ...
    static {
        ...
        // The content URI of the form "content://com.example.android.inventoryApp/book" will map to the
        ...
        // For example, "content://com.example.android.inventoryApp/book/3" matches, but
        // "content://com.example.android.inventoryApp/book" (without a number at the end) doesn't match.
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case BOOK_ID:
                ...
                // For an example URI such as "content://com.example.android.inventoryApp/book/3",
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        if (name == null) {
            throw new IllegalArgumentException("You must insert a correct name.");
        }

        // Check that the price is valid
        Integer price = values.getAsInteger(BookEntry.COLUMN_BOOK_PRICE);
        if (price == null || price < 0 ) {
            throw new IllegalArgumentException("You must insert a correct price.");
        }

        // Check that the price is valid
        Integer quantity = values.getAsInteger(BookEntry.COLUMN_BOOK_QUANTITY);
        if (quantity == null || quantity < 0 ) {
            throw new IllegalArgumentException("You must insert a correct amount.");
        }

        // Check that the supplier is not null
        String supplier = values.getAsString(BookEntry.COLUMN_BOOK_SUPPLIER);
        if (supplier == null) {
            throw new IllegalArgumentException("You must insert a valid supplier.");
        }

        // Check that the number is not null
        String number = values.getAsString(BookEntry.COLUMN_BOOK_PHONE);
        number.replaceAll("\\D", "");
        if (number == null || number.length() !=10) {
            throw new IllegalArgumentException("You must insert a valid number.");
        }
